...
router.post(
  ...
  [
    ...
    body("message")
      ...
      .withMessage("Mesaj uzunluğu 1 ile 1000 arasında olmalıdır"),
  ],
  upload.array("files"),
  async (req, res, next) => {
    try {
      ...
      if (created_from !== task.related_person._id)
...
router.put(
  ...
  async (req, res, next) => {
    try {
      ...
      if (name !== null) existingConversation.name = name;

      if (task_id !== null) existingConversation.task_id = task_id;
